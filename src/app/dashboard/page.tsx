...
const metrics = [
  {
    title: "Verified Consignees",
    value: "11 Enterprises",
    change: "100% Statutory",
    changeLabel: "19 U.S.C. § 1431",
    trend: "up" as const,
    icon: Users,
  },
  {
    ...
    value: "20 Active",
    change: "4 HS Codes",
    changeLabel: "UN Comtrade / USITC",
    trend: "up" as const,
    icon: Globe,
  },
  {
    title: "Manifest Filings",
    value: "14 Verified BoLs",
    change: "ACE / CBP",
    changeLabel: "Customs Registry",
    trend: "up" as const,
    icon: Layers,
  },
  {
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#121215] rounded-lg border border-[#eaeaea] dark:border-[#27272a] p-6">
        ...
        <div className="space-y-3">
          {[
            {
              action: "BoL Manifest Registered",
              detail: `${BUYERS_DATA[0]?.shipments[0]?.bolNumber || "MSCU2489102"} — 18,450 kg "${BUYERS_DATA[0]?.name} (HTSUS 6302.31.90)"`,
              market: "Nhava Sheva (INNSA) → Port of Long Beach (USLGB 2709) • MSC GÜLSÜN (Voy 241E)",
              time: "2 min ago",
            },
            {
              action: "Customs Entry Cleared",
              detail: `${BUYERS_DATA[1]?.shipments[0]?.bolNumber || "EGLV9182304"} — 24,800 kg "${BUYERS_DATA[1]?.name} (HTSUS 6302.32.10)"`,
              market: "Nhava Sheva (INNSA) → Port of Los Angeles (USLAX 2704) • CBP Form 7501 Cleared",
              time: "14 min ago",
            },
            {
              action: "Ocean Carrier Manifest Filed",
              detail: `${BUYERS_DATA[2]?.shipments[0]?.bolNumber || "CMDU8921045"} — 31,200 kg "${BUYERS_DATA[2]?.name} (HTSUS 6302.60.00)"`,
              market: "Nhava Sheva (INNSA) → Port of Savannah (USSAV 1703) • CMA CGM DALILA (Voy 04F)",
              time: "38 min ago",
            },
            {
              action: "Port Arrival Notice",
              detail: `${BUYERS_DATA[5]?.shipments[0]?.bolNumber || "HLCU9182301"} — 19,800 kg "${BUYERS_DATA[5]?.name} (TARIC 6302.60.00)"`,
              market: "Mundra Port (INMUN) → Port of Hamburg (DEHAM) • Hapag-Lloyd AL ZUBARA (Voy 082W)",
              time: "52 min ago",
            },
            {
              action: "Customs Inspection Released",
              detail: `${BUYERS_DATA[7]?.shipments[0]?.bolNumber || "MAEU7192043"} — 22,100 kg "${BUYERS_DATA[7]?.name} (UK Tariff 6302.31.00)"`,
              market: "Nhava Sheva (INNSA) → Port of Felixstowe (GBFXT) • Maersk Mc-Kinney Møller (Voy 19B)",
              time: "1 hour ago",
            },
          ].map((item, i) => (
            <div
